  ...
  const load = useCallback(async () => {
    ...
    try {
      const [f, b] = await Promise.all([
        ...
        apiFetch<BankAccount[]>('/finance/bank-accounts', { token }),
      ]);
      setFunds(f);
      setBanks(b);
    } catch (err) {
      ...
      {tab === 'funds' && (
        <>
          {editing && (
            ...
            >
              <h2 style={{ fontSize: '1.05rem' }}>Редагувати: {editing.type}</h2>
              <div>
                <label>Назва</label>
          ...
          <section className="card">
            {loading ? (
              ...
              <EmptyState title="Фондів немає" description="Пройдіть майстер налаштування ОСМД." />
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <div style={{ color: 'var(--muted)', fontSize: '0.85rem' }}>
                        {f.type} · початковий {formatMoney(f.openingBalance)}
                        {f.bankAccount
                          ? ` · ${f.bankAccount.bankName} ${f.bankAccount.iban}`
                          : ' · без IBAN'}
                      </div>
                    </div>
                    <button type="button" className="btn btn-sm btn-ghost" onClick={() => startEdit(f)}>
                      Редагувати
                    </button>
                  </li>
                ))}
